...
use grraf::backend::{Graph, IndexList, NodeId};
...
fn create_five_letters_as_node_ids() -> Vec<NodeId> {
    vec!["A", "B", "C", "D", "E"]
        .iter()
        .map(|letter| node_id!(name: **letter))
        .collect()
}
...
fn create_exemple_line_index_list() -> IndexList<usize, ()> {
    ...
    graph
        .connect_vertices(node_id!(name: "A"), (), node_id!(name: "B"))
        .unwrap();
    graph
        .connect_vertices(node_id!(name: "B"), (), node_id!(name: "C"))
        .unwrap();
    graph
        .connect_vertices(node_id!(name: "C"), (), node_id!(name: "D"))
        .unwrap();
    graph
        .connect_vertices(node_id!(name: "D"), (), node_id!(name: "E"))
        .unwrap();
...
fn dfs_line_graph() {
    ...
    assert_eq!(
        depth_first_search(&graph, Some(node_id!(name: "A")), 4),
        Some(ids)
    );
}
...
fn dfs_random_graph() {
    ...
    graph
        .connect_vertices(node_id!(name: "A"), (), node_id!(name: "C"))
        .unwrap();
    graph
        .connect_vertices(node_id!(name: "A"), (), node_id!(name: "B"))
        .unwrap();
    ...
    graph
        ...
        .unwrap();
    graph
        .connect_vertices(node_id!(name: "C"), (), node_id!(name: "E"))
        .unwrap();
    graph
        .connect_vertices(node_id!(name: "E"), (), node_id!(name: "A"))
        .unwrap();
    graph
        .connect_vertices(node_id!(name: "E"), (), node_id!(name: "B"))
        .unwrap();
